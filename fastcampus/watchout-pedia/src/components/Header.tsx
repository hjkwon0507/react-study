...
const MenuButton = styled.button<{ active?: boolean }>`
  font-size: 15px;
  color: ${({ active }) => active ? 'rgb(53, 53, 53)' : 'rgb(126, 126, 126)'}; // active 일 경우 : 아닐 경우
  cursor: pointer;
  border: none;
  background: none;
`;
...
const TextLogo = styled.h1`
  font-size: 24px;
  font-weight: 700;
  > span[class="primary"] { // span의 class가 primary 인 경우
    color: rgb(255, 47, 110);
  }
  > span:not(.primary) { // span의 class가 primary 아닌 경우
    color: #222;
  }
`;
